...
- **`GSAI_DACH`** — Generator połaci dachu ze straight-skeleton: wskaż obrys → połacie, kalenice, krawędzie, opisy i strzałki spadków. Odczyt obrysu native-first + jawne rzutowanie przy encji z zapisanego pliku (BUG-10-świadome; 2026-09-18, build 260827 — COM wycięty, BUG-13). Generowania połaci z obrysu nie ma nikt.
...
- **`GSAI_POLA`** — Pole i obwód pól/pomieszczeń: zaznacz oknem albo wskaż pomieszczenie → opis w centroidzie + tabela zbiorcza + eksport CSV. Liczy też na pliku ZAPISANYM od klienta — obejście BUG-10 przez jawne rzutowanie (gsai_downcast_core.as_typed, 2026-09-18, build 260827). *Dawny zapis mówił o ścieżce COM; COM jest martwy (BUG-13) i został z kodu wycięty.* Wchłonęło ZESTAWIENIE i PRZEDMIAR (konsolidacja, Robert ✓).
...
- `GSAI_SKARPA` — Oznaczenia skarpy między górnym a dolnym zarysem — kreski długie/krótkie na przemian, rozstaw i długość wg skali rysunku; zarysy = dowolne krzywe (linia/polilinia/łuk). Odczyt zarysów native-first + jawne rzutowanie (BUG-10-świadome; 2026-09-18, build 260827 — dawny zapis o fallbacku COM bez castu jest odwrócony: cast naprawiony w 260813, COM martwy wg BUG-13). Wniosek klienta (P. Szczepański).
