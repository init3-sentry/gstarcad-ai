...
Instrukcja systemowa pomocnika „GstarCAD Python Helper" zawiera następujące elementy. Pełen tekst znajdziecie w pliku `gstarcad-ai-wewnetrzne/produkt-i-badania/poc-plugin-askai/backend/system-prompt.md` w repozytorium wewnętrznym (poproś Dawida o kopię, jeśli jej nie masz) — to jest też dokument do przeczytania i ewentualnej korekty.
...
Postępuj tak. Otwórz nowy chat w dowolnym modelu. W pierwszym polu wpisu **wklej cały nasz przewodnik systemowy** (skopiowany z pliku `gstarcad-ai-wewnetrzne/produkt-i-badania/poc-plugin-askai/backend/system-prompt.md` w repozytorium wewnętrznym (poproś Dawida o kopię, jeśli jej nie masz)). Na końcu wklejonego tekstu dodaj swoje pytanie. Naciśnij wyślij.
