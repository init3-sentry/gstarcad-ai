...
Maintained by **TMSys** (the official GstarCAD distributor for Poland) for the `gstarcad-ai` project. Version **2.2**, dated **2026-07-10**.
...
**What changed in v2.2 (2026-07-10):** **retracted a wrong claim.** v2.1 stated the `GcDb2dPolyline` crash was merely *environmental instability over RDP*. Cross-machine team testing (3 independent machines, 2027 without SP1, local not RDP, every time) proves that **programmatic construction** of `GcDb2dPolyline` (`appendVertex`) is a **genuine, reproducible GstarSoft defect** that crashes on regen. Added as pitfall #8; the verification annex now separates *reading* a native 2dPolyline (works) from *constructing* one (crashes). Reading remains confirmed working.
...
| 8 | `GcDb2dPolyline()` + `appendVertex(GcDb2dVertex(...))` to build a heavy 2D polyline | append returns `eOk`, but GstarCAD **crashes to desktop on the next regen**, no message. Reproduced on 4 machines incl. 3 local non-SP1 (2026-07-10) — a real GstarSoft defect, not environment | build polylines with the lightweight **`GcDbPolyline`** (2D) + `addVertexAt` (confirmed working everywhere). Reading an existing/native `GcDb2dPolyline` is fine — only programmatic construction crashes |
...
- 🟢 **Reading an existing `GcDb2dPolyline`'s vertices is confirmed working** (2026-07-10, SP1 / R27.1.0.2606, via `sweep-7-verify.py` step-by-step isolation): `gcedEntSel` / model-space iteration → `gcdbOpenObject` → `isKindOf(GcDb2dPolyline.desc())` → `vertexIterator()` → `GcDb2dVertex.cast().position()` reads all vertices of a **natively drawn** polyline cleanly. Requires `SETVAR PLINETYPE 0` to have a real `GcDb2dPolyline` in the drawing (else the lightweight `GcDbPolyline` is created, read via `numVerts`/`getPointAt` — not yet exercised here).
- 🟢 **Programmatic construction of `GcDb2dPolyline` crashes GstarCAD to desktop — confirmed reproducible defect** (2026-07-10, cross-machine team tests, see pitfall #8). `GcDb2dPolyline()` + `appendVertex(GcDb2dVertex(...))` returns `eOk`, iteration reads the vertices, but GstarCAD crashes on the next **regen** with no message. Reproduced on **4 machines**: our LC (2027 SP1, R27.1.0.2606, over RDP) and three independent machines (2027 **without SP1**, R27 build 260527), across editions (Plus/Pro), GPUs (GTX 1650 / integrated Radeon / RTX 4060), Windows 10 and 11, **locally (not over RDP), every time.** **Correction / retraction (2026-07-10):** a previous version of this document claimed the "2dPolyline crashes" were merely *environmental instability over RDP*. That was an over-generalization and is **retracted** — the three local non-SP1 machines prove the construction crash is a genuine, environment-independent GstarSoft defect. Reading a natively drawn 2dPolyline (bullet above) is unaffected. Reported to GstarSoft via William 2026-07-10; awaiting R&D. Workaround: build with the lightweight 2D `GcDbPolyline` + `addVertexAt` (confirmed working on all machines).
- 🔴 **Unverified / in progress:** GstarSoft R&D answer to the `GcDb2dPolyline` / `GcDb3dPolyline` crash reports; systematic API diff GstarCAD 2026 vs 2027; angular/diametric dimensions, boolean/trim operations; lightweight `GcDbPolyline` vertex read (`numVerts`/`getPointAt`). When the user's request depends on a 🔴 item, generate the closest 🟡/🟢 pattern and add a one-line Polish comment that the pattern awaits final verification.
