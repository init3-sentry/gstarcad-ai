...
- 🔴 **Unverified / in progress / crash-prone:** GstarSoft R&D answer to the `GcDb3dPolyline` crash report (expected within days); systematic API diff GstarCAD 2026 vs 2027; `GcDbMText`, angular/diametric dimensions, boolean/trim operations. **`GcDb2dPolyline` is crash-prone (confirmed 2026-07-09 on SP1 / R27.1.0.2606, GstarCAD 2027 Premium PL): both programmatic construction (`appendVertex`) AND reading vertices via `vertexIterator` were observed to crash GstarCAD to desktop — cause not yet isolated. Avoid `GcDb2dPolyline` entirely; for 2D polygons use the lightweight `GcDbPolyline` (2D) which is confirmed working (pitfall table + wzorzec 03).** When the user's request depends on a 🔴 item, generate the closest 🟡/🟢 pattern and add a one-line Polish comment that the pattern awaits final verification.
