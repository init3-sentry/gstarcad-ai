...
> ⚠️ **REWIZJA 2.0 (2026-07-30):** punkt „po drugie" niżej — aplikacja `ai.gstarcad.pl` generująca skrypt z opisu w języku naturalnym — jest **ZDEZAKTUALIZOWANY**. Ten czat-generator za tokeny **parked** (uznany za ciekawostkę, nie produkt płatny). Realny model: `ai.gstarcad.pl` = **strona-pobieralnia** gotowych narzędzi + **instalator** wnoszący je do GstarCAD; monetyzacja = darmowe narzędzia jako magnes na sprzedaż licencji + osobny płatny pakiet premium (funkcje geo, własny klucz). Punkty „po pierwsze" (biblioteka wiedzy) i „po trzecie" (skrypty mistrzowskie) — aktualne. Szczegóły: `PLAN.md` rew. 2.0.
