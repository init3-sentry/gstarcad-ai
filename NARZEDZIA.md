...
| **`GSAI_RZEDNE`** | Znacznik rzędnej wysokościowej (±0.00) z auto-odczytem współrzędnej. Natywnie brak dedykowanego. Generatywne. | kod w *powertools* | 🟡 zbudowane; **status testu zespołu do potwierdzenia** (nie zweryfikowane w tej rundzie). |
...
| **`GSAI_RATUNEK`** | Reset interfejsu — przywraca znikające paski i wstążki jednym poleceniem (ratunek po uszkodzonym CUI). | kod w *powertools* | 🟡 zbudowane (task REPAIR-CUI); **status testu do potwierdzenia**. |
...
| **`GSAI_ORNAMENT`** | Demonstracyjny generator wzorów geometrycznych (algorytm, nie AI). | kod w *powertools* | ⏸️ **PARKED 06.08** — Robert ([robert#9](https://github.com/init3-sentry/gstarcad-ai-robert/issues/9)): „nie widzę zastosowania". **Nie na stronę** (nazwa „Generator AI" łamie ramę: klient nie dostaje przycisku AI). |
