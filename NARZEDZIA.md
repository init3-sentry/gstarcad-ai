...
> _Stan na 2026-08-27._ **To jest jedyny obowiązujący katalog narzędzi.** Wszystkie repozytoria projektu linkują tutaj. Zmiana statusu, nazwy komendy albo nowe narzędzie — **zmienia się tylko ten plik**.
>
...
> - _(`GSAI_KOL` był tu jako „usunięte, bo DRAWORDER" — **wrócił reworkiem FAZA 2** jako menedżer kolejności per-warstwa; status w sekcji 🟡 niżej.)_
>
...
> ### 🆕 Aktualizacja 2026-08-24 — wspólny renderer tabel + SCHODY v4 + szybszy start
>
> Release auto-update: **`2026.08.24-c8f831d`** na /web (wdrożone, verify OK). Instalka .exe przebudowana na LC (SHA256 `c3eae111…`), wgrana jako `GSAI-Setup-2026-08-22.exe` w [zespol/instalka-2026-08-22](https://github.com/init3-sentry/gstarcad-ai-zespol) i robert/reinstall-2026-08-22.
>
> - **Wspólny renderer tabel (`gsai_tabela_core`) — fundamentalny fix.** Dotyczy wszystkich narzędzi tabelowych (POLA / ZLICZ / GRANICA / TIN / SCHODY). Wysokość tekstu ustawiana per komórka, realna szerokość znaku 0,67h (koniec pustych boków i „absurdalnie szerokich" tabel przy dużej wysokości tekstu), pionowe wyśrodkowanie komórek, opcjonalne zawijanie długich kolumn. **POLA/ZLICZ bez zmiany zachowania** (błąd ujawniał się dopiero przy dużym h, jak w SCHODY) — geodezja bez regresji, potwierdzone. Przed premierą i tak obowiązuje re-test w instalce.
> - **`GSAI_SCHODY` v4** — tabela zgodności także dla łuku (parytet 4/4), rysowanie na warstwie bieżącej zamiast dedykowanych warstw SCHODY-\* (wymóg Roberta z robert#9), numery stopni wyśrodkowane, pozioma etykieta „N × tread". **Wgrane, czeka na praktyczny werdykt Roberta** — nie przenosimy do „gotowe" na podstawie samego wdrożenia.
> - **Start GstarCAD szybszy o ~1,5–2 s** — mechanizm ładowania (`GSAI_AUTOSTART`) pomija regen + CUILOAD wstążki, gdy sygnatura (manifest + ulubione) się nie zmieniła; self-heal przy uszkodzonym `cuix`, awaryjnie `GSAI_CUI`. Zweryfikowane na LC. To mechanizm wewnętrzny (nie komenda w katalogu).
>
> Opisy narzędzi dla SCHODY (2 wersje) odświeżone jako **szkic do odbioru Roberta** — nie jako opis gotowy na stronę.

> ### 🆕 Aktualizacja 2026-08-24 (wieczór) — audyt WT schodów (robert#18), ORTOFOTO poza premierę, obejście TRASA/SCHRON
>
> - **`GSAI_SCHODY` — rozbudowa kontroli zgodności WT (audyt [robert#18](https://github.com/init3-sentry/gstarcad-ai-robert/issues/18)).** Balustrada §298 teraz **egzekwowana** (czerwony, gdy wpisana wysokość < wymóg per typ — wcześniej tylko opisowa); tabela zgodności obejmuje też schody **zabiegowe** (§69.6, parytet z łukiem); nowe pole okna **„Usytuowanie: wewnętrzne / zewnętrzne"** → dla zewnętrznych **§69 max 10 stopni w biegu** (niezależnie od kategorii) + **§69.5 s≥35 cm** dla zewnętrznych wielorodzinnych/UP przy głównym wejściu; **żółte ostrzeżenie praktyczne >17 stopni** dla kategorii zwolnionych z limitu (jednorodzinne/rekreacja). Norma bez zmian — WT Dz.U. 2022/1225, §-y już zabezpieczone w rejestrze. **Status SCHODY bez zmian (zaakceptowane, premierowy).** Opisy (2 wersje) odświeżone jako **szkic do odbioru Roberta**.
> - **`GSAI_ORTOFOTO` → odsunięte poza premierę 1.09** (decyzja Dawida 24.08). Raster + bug producenta BUG-05 + downcast 162444 na słowniku obrazów. Przeniesione do sekcji *Odsunięte poza premierę*. Wróci z fixem GstarSoftu (I tydzień września). **Nie „gotowe".**
> - **`GSAI_TRASA` + `GSAI_SCHRON` — obejście buga 162444 (Python) / 162495 (COM)** potwierdzonego przez GstarSoft: czytają wierzchołki/pole przez **ADS DXF (grupa 10)**, gdy encja wraca jako klasa bazowa na wczytanym pliku. **Obejście POTWIERDZONE przez Tomasza na wczytanych plikach PL-2000** (TRASA: 2 przecięte działki; SCHRON: pole 52,43 m²). Wracają na premierę.
> - **Instalator** (info dla marketingu/grafików, nie zmiana narzędzia): wpięte grafiki kreatora — mała (płaski AI, `wizard-small.png`) + duży baner Niny (`wizard-large.png` 534×1022) + ikona `gsai.ico`.

> ### 🆕 Aktualizacja 2026-08-27 — batch werdyktów Roberta „zamykamy" (#18/#22)
>
> Robert domknął na produkcję (cytaty w komórkach „Zwalidował/Stan"): **`GSAI_PN`** ([robert#18](https://github.com/init3-sentry/gstarcad-ai-robert/issues/18): „Zamykamy. Jest OK"), **`GSAI_WSS`** (Warstwy Standard Short) ([robert#18](https://github.com/init3-sentry/gstarcad-ai-robert/issues/18): WARSTWY SHORT „Działa super, Zamykamy temat"), **`GSAI_FORMATKA`** i **`GSAI_PODZIALKA`** ([robert#22](https://github.com/init3-sentry/gstarcad-ai-robert/issues/22): „jest OK. Zamykamy"). Przeniesione do sekcji ✅.
>
> **Rozbicie „Warstwy Standard" na dwie komendy:** narzędzie dostarczamy jako `GSAI_WSS` (Short — gotowe, wyżej) oraz `GSAI_WSF` (Full — pełny branżowy zestaw, kolory kolejne z indeksu 1–255). Wariant `GSAI_WSF` czeka na finalny zrzut Roberta — zostaje w sekcji 🟡, na stronę nie idzie. Stara pojedyncza nazwa `GSAI_WARSTWY_STANDARD` już nie obowiązuje (realne komendy to WSS/WSF).
>
> **NIE domknięte (zostają w testach/naprawie):** `GSAI_SCHODY` — domyślny łuk poza §69.6, Robert wybiera wariant startowy ([robert#18](https://github.com/init3-sentry/gstarcad-ai-robert/issues/18)); `GSAI_TABELKA` — rozpad bloku po usunięciu Logo + brak atrybutów PROJEKTOWAŁ 1/2/3 ([robert#22](https://github.com/init3-sentry/gstarcad-ai-robert/issues/22)). Testy Wiktora/zespołu (smoke/self-test) **nie** kwalifikują do „gotowe" — to robi wyłącznie werdykt Roberta.

> ### 🆕 Aktualizacja 2026-09-04 — GSAI_WMS (podkład z geoportalu, darmowy magnes) · WYDANE 2026-09-09
>
> Nowe narzędzie **`GSAI_WMS`** — podkład z usług WMS wprost w rysunku, na prawdziwych współrzędnych: **ortofotomapa geoportalu GUGiK + granice/numery działek EGiB + własny URL WMS**. Okno wyboru usług + rozdzielczość, obszar wskazywany prostokątem. Zamyka lukę wobec ZWCAD (ma ortofoto z serwera natywnie, GstarCAD nie) i przejmuje popyt płatnej wtyczki Wms2Cad (599–799 zł/rok za WMS-do-GstarCAD). **Pozycjonowanie: darmowe (magnes)** — dane geoportalu są publiczne (INSPIRE 2007/2/WE + ustawa IIP Dz.U. 2010/76/489 + art. 40a Prawa geodezyjnego). Działka jako geometria do liczenia zostaje w płatnym module geodezyjnym (podział darmo=pokaż / płatne=policz).
> **Wydane 2026-09-09** (release `2026.09.09-997f527`, .pyd) po walidacji runtime end-to-end (Wiktor 2026-09-08 + Dawid naocznie 2026-09-09, GstarCAD 260827). Status w katalogu = 🟡 do formalnego odbioru Roberta („kontekst działki") — na stronę dopiero po jego werdykcie.

> ### 🆕 Aktualizacja 2026-09-09 — GSAI_WMS w pełni działa · GSAI_METRYKA wycięta z wydania
>
> Release **`2026.09.09-997f527`** (podpisana instalka `GSAI-Setup-2026-09-09.exe` + /web) zamienia martwy/zdegradowany kafelek WMS w **flagową, działającą funkcję**. Naprawione i potwierdzone runtime (Wiktor 08.09 + Dawid naocznie 09.09, GstarCAD 260827):
> - **przezroczystość** — ortofoto jest **widoczne pod granicami/numerami działek** (wcześniej działki zasłaniały zdjęcie);
> - **natywny układ rysunku** — podkład pobierany i osadzany w układzie rysunku (PL-2000: strefy 2176–2180), bez reprojekcji → sąsiednie rastry schodzą się równo na styku;
> - **usuwanie nakładek** — nowy podkład kasuje stary nakładający się (koniec „bieli" ze stosu przezroczystych rastrów), a osobne obszary współistnieją;
> - **wybór jakości w oknie** — Szybka (podgląd) / Standardowa (czytelne numery, domyślna) / Wysoka (do wydruku).
>
> **`GSAI_METRYKA` — wycięta z tego wydania.** Weszła wcześniej przypadkiem i zepsuta; po `GSAI_UPDATE` znika z instalacji klienta (komenda „nieznane polecenie"). Wraca osobno po naprawie (tor PRO). Nie figuruje jako gotowa/wydana w żadnym materiale klienckim.
>
> WMS zostaje **🟡** (wydane + runtime-pass, geodezja idzie na akcept Dawida jak `GSAI_GEORASTER`/`GSAI_GML`) — na **publiczną stronę** dopiero po formalnym odbiorze „kontekst działki". Opis (2 wersje) w podręczniku/katalogu odświeżony.

**Komendy mają przedrostek `GSAI_`** (zmiana z 14.07.2026). Wpisuje się np. `GSAI_XYZ`. Stare nazwy bez przedrostka już nie działają — po pobraniu nowej wersji trzeba zrobić `APPLOAD` jeszcze raz.
...
|---|---|---|---|
| **`GSAI_ZNW`** 🦸 | Hurtowa zmiana nazw warstw wzorcem (find→replace w środku nazwy) z obsługą kolizji. Natywnie GstarCAD tego nie ma — **lukę potwierdził na piśmie QA Manager Autodesku** (ADR 08). Wartość = hurt/wzorzec, nie pojedyncza warstwa. | źródło [31_rename_warstw_wzorcem.py](biblioteka-rag/przyklady/31_rename_warstw_wzorcem.py) → **doportowane do instalatora 2026-08-12** (`skrypty/GSAI_ZNW.py`, AST+stuby 🟢) | Tomasz 29.07 (hurt); **re-test w instalce przed premierą** |
| **`GSAI_XYZ`** | Plik z Excela/Notatnika ze współrzędnymi → punkty z numerami. Natywnie brak (płatne nakładki = dowód popytu). | źródło [25_import_coordinates.py](biblioteka-rag/przyklady/25_import_coordinates.py) → **doportowane do instalatora 2026-08-12** (`skrypty/GSAI_XYZ.py`, AST+stuby 🟢) | Robert; **re-test w instalce przed premierą** |
...
| **`GSAI_SCHODY`** 🎁 | Generator schodów (rzut / łuk / przekrój; tryby biegu) — „wow": schody w GstarCAD za darmo. **Rysuje też po ponownym otwarciu pliku** (generatywne → odporne na BUG-10). | kod w repo *powertools*, dostawa: release `skrypty-test` | Tomasz 31.07; **v2 06.08** (cm zamiast mm, wynik jako blok, auto-opis); **v3 15.08 po robert#16**: opis czyta się wzdłuż biegu, pełna strzałka na łuku (Robert ✓), czerwone podświetlenie niezgodności z normą. **v4 24.08:** tabela zgodności także dla łuku (parytet 4/4: rzut/przekrój/pochylnia/łuk), rysowanie na **warstwie bieżącej — zero warstw SCHODY-\*** (wymóg [robert#9](https://github.com/init3-sentry/gstarcad-ai-robert/issues/9), separacja kolorem encji), numery stopni wyśrodkowane, etykieta „N × tread" pozioma pod schodami, dynamiczny cap kolumny „Wynik". Zmiany wgrane — **czekają na praktyczny werdykt Roberta** (nie potwierdzone na realnym rysunku przez praktyka). Zostaje od Roberta: „typ opisów wg normy" + opcja opisów jako atrybuty. **v5 24.08 (audyt WT robert#18):** balustrada §298 egzekwowana (czerwony, gdy wpisana wys. < wymóg per typ), tabela zgodności także dla schodów zabiegowych (§69.6, parytet z łukiem), pole „Usytuowanie: wewn/zewn" → dla zewnętrznych §69 max 10 stopni w biegu + §69.5 s≥35 cm (wielorodzinne/UP przy głównym wejściu), żółte ostrzeżenie praktyczne >17 stopni dla kategorii zwolnionych (jednorodzinne/rekreacja). Norma: WT Dz.U. 2022/1225 (bez nowego aktu). |
| **`GSAI_PN`** 🧭 | Ozdobna strzałka północy — **6 stylów dwutonowych** (prosta/strzałka/romb/róża wiatrów/kompas geodezyjny/iglica), panel wyboru z podglądem + wysokość + klik. **v2 06.08: wynik jako blok na bieżącej warstwie** (obrót przez ROTATE, przesuń/kasuj jako jeden obiekt). Natywnie brak (GstarCAD ma tylko `COMPASS`/`NORTHDIRECTION`). Generatywne → BUG-10-safe. `GSAI_STRZALKA_GALERIA` = wszystkie naraz. | kod w *powertools*, release `skrypty-test` | LC 01.08 → **zespół 06.08 (Tomasz ✓, [#79](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/79))** → ✅ **Robert [robert#18](https://github.com/init3-sentry/gstarcad-ai-robert/issues/18) 2026-08-26: „Zamykamy. Jest OK" — na produkcję** |
...
| **`GSAI_SKARPA`** 🆕 | Oznaczenia skarpy między górnym a dolnym zarysem — kreski długie/krótkie na przemian, rozstaw i długość wg skali rysunku; zarysy = dowolne krzywe (linia/polilinia/łuk). Native-first + COM fallback (BUG-10-świadome, bez castu). Wniosek klienta (P. Szczepański). | kod w *powertools* (`GSAI_SKARPA.py` + `gsai_skarpa_core.py`, self-test 6/6) | **runtime ✓ Wiktor 08.09**; promocja do wydania 10.09 (Dawid); odbiór praktyka (tor terenowy/geodezja, nie Robert) w toku |
...
| **`GSAI_CUI`** 🔧 | „Odzyskaj wstążkę" — naprawa interfejsu GstarCAD, gdy zniknie wstążka/menu (uszkodzony `gcad.cuix` / porwany rejestr `MenuFile`). Przywraca z baseline sprzed instalacji albo z 5 zdrowych snapshotów (ring), naprawia rejestrację głównego menu; okno z przyciskiem „Odzyskaj wstążkę" + modal „ZRESTARTUJ GstarCAD". Naprawa interfejsu, nie danych — rysunki nietknięte. Alias globalny: GSAI_REPAIRCUI. | kod w *powertools* (`skrypty/GSAI_CUI.py` + `gsai_cui_core.py`, integracja `GSAI_AUTOSTART.py`; self-test core 16/16) | ✅ **ZATWIERDZONE (Dawid 2026-08-23)** — narzędzie serwisowe (kategoria **Serwis**), **wchodzi na stronę bez kolejki Roberta**. Zwalidowane e2e na LC 22.08 (repro: hijack `MenuFile` → restore → wstążka wraca), na produkcji (release `2026.08.22-cbaf250`, auto-load bez APPLOAD). |
| **`GSAI_FORMATKA`** 📐 | Sama ramka rysunkowa **ISO 5457** (margines 20 mm lewy / 10 mm pozostałe), formaty A4/A3/A2 (A4 też poziomo), guard „tylko Arkusz", BEZ tabliczki (każda firma ma własną). Natywnie brak w bazie Professional. Generatywne → BUG-10-safe. | kod w *powertools*, release `skrypty-test` | ✅ **ZATWIERDZONE — Robert [robert#22](https://github.com/init3-sentry/gstarcad-ai-robert/issues/22) 2026-08-26: „jest OK. Zamykamy temat" — na produkcję.** Ramka 0,5 mm fizyczną szerokością, Jakub ✓ [zespol#121](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/121). |
| **`GSAI_PODZIALKA`** 📏 | Podziałka liniowa (skala rysunku) na arkuszu — rysowana w mm w Przestrzeni Papieru. Generatywne → BUG-10-safe. | kod w *powertools* (`GSAI_PODZIALKA.py`) | ✅ **ZATWIERDZONE — Robert [robert#22](https://github.com/init3-sentry/gstarcad-ai-robert/issues/22) 2026-08-26: „jest OK. Zamykamy" — na produkcję.** Render potwierdzony przez zespół ([zespol#95](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/95), fix nachodzenia „10m"). |
| **`GSAI_WSS`** 🗂️ | **Warstwy Standard Short** — praktyczny zestaw 42 warstw jednym poleceniem (ustawienia praktyka wg wzorca Roberta: grupy A/E/S/L/W + systemowe, gotowe kolory ACI+RGB, typy linii i grubości pod codzienną robotę). Zakłada tylko te, których w rysunku jeszcze nie ma. Natywnie brak. Generatywne → BUG-10-safe. Alias globalny: `GSAI_LAYERSTD_SHORT`. | kod w *powertools* (`skrypty/GSAI_WSS.py` + `gsai_warstwy_std_core.py`) | ✅ **ZATWIERDZONE — Robert [robert#18](https://github.com/init3-sentry/gstarcad-ai-robert/issues/18) 2026-08-26: WARSTWY SHORT „Działa super, Zamykamy temat" — na produkcję.** Kafelek Niny + opis (2 wersje) gotowe. |
...
|---|---|---|---|
| **`GSAI_WMS`** 🆓 🆕 | Podkład z usług WMS wprost w rysunku, na prawdziwych współrzędnych: ortofotomapa geoportalu GUGiK + granice/numery działek EGiB + własny URL WMS. Okno wyboru usług + **jakość podkładu** (Szybka–podgląd / Standardowa–czytelne numery, domyślna / Wysoka–do wydruku), obszar = prostokąt (dwa naroża). Silnik osadzania wspólny z `GSAI_GEORASTER`; obraz siada na współrzędnych, nowy podkład kasuje stary nakładający się, osobne obszary współistnieją (multi-raster ≥260827). Darmowy magnes: dane geoportalu publiczne (INSPIRE 2007/2/WE + ustawa IIP Dz.U. 2010/76/489 + art. 40a PGiK). Przejmuje popyt płatnej wtyczki Wms2Cad (599–799 zł/rok). Kod chroniony (.pyd), zależność `gsai_geoportal_core`. | bundle produkcyjny (release `2026.09.09-997f527`, .pyd) | 🟡 **WYDANE 2026-09-09** w podpisanej instalce `GSAI-Setup-2026-09-09` + /web. **Zwalidowane runtime end-to-end** (Wiktor 2026-09-08 + Dawid naocznie 2026-09-09, GstarCAD 260827: ortofoto **widoczne pod** granicami/numerami działek, podkład pobrany i osadzony w natywnym układzie rysunku PL-2000 bez reprojekcji → rastry schodzą się na styku, nowy podkład kasuje stary nakładający się, wybór jakości). **Formalny odbiór Roberta („kontekst działki") wciąż otwarty → na stronę dopiero po jego werdykcie.** Opis (2 wersje) + kafelek = szkic do odbioru Roberta. |
| **`GSAI_WSF`** 🗂️ | **Warstwy Standard Full** — pełny branżowy zestaw warstw wg normy (A/K/instalacje/Z + systemowe), kolory kolejne z palety indeksu ACI 1–255 do własnego dostrojenia. Panel wyboru branż. Rodzeństwo `GSAI_WSS` (Short). Natywnie brak. Generatywne → BUG-10-safe. Alias globalny: `GSAI_LAYERSTD_FULL`. | kod w *powertools* (`skrypty/GSAI_WSF.py` + `gsai_warstwy_std_core.py`) | 🟡 **Zbudowane, kafelek Niny + opis (szkic) gotowe. Czeka na finalny zrzut Roberta** — do jego werdyktu zostaje poza stroną (na produkcję idzie tylko wariant Short). |
...
| **`GSAI_CHROP`** | Symbol chropowatości powierzchni (haczyk 60°, warianty usunięcia materiału, półka na dane). | źródło [27_surface_roughness.py](biblioteka-rag/przyklady/27_surface_roughness.py) → **doportowane do instalatora 2026-08-12** (`skrypty/GSAI_CHROP.py`, AST+stuby 🟢) | 🟡 **Do testów zespołu ([zespol#67](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/67))** — norma potwierdzona (**PN-EN ISO 21920-1:2022** zastąpiła ISO 1302; symbol przeniesiony bez zmian, wartość=wolny tekst). Czeka wizualny werdykt Roberta. Generatywne → BUG-10-safe. |
| **`GSAI_RZUT`** | Tabliczkowy symbol metody rzutowania 1./3. kąta (ISO 5456-2) — ścięty stożek w dwóch widokach. Natywnie brak. Generatywne. | kod w *powertools* | 🟡 **06.08 przemianowane z `GSAI_RZUT`** (stara nazwa myliła — Robert robert#9). Do testu. |
...
| **`GSAI_NUM`** | Automatyczne wstawianie i inkrementacja numerów rysunków/arkuszy. | kod w *powertools* | 🟡 zbudowane; **status testu do potwierdzenia**. |
| **`GSAI_LEGW`** | Wstawia legendę warstw jako tabelę na rysunku: nazwa + próbka koloru + próbka typu linii (graficznie) + szerokość + druk + opis. Pomysł Roberta ([robert#13](https://github.com/init3-sentry/gstarcad-ai-robert/issues/13)). Natywnie brak. Generatywne → BUG-10-safe. | kod w *powertools* (`skrypty/GSAI_LEGW.py` + `gsai_legenda_core.py`, self-test 11/11) | 🟡 zbudowane; **w testach zespołu, na stronę po odbiorze praktyka**. |
| **`GSAI_KOL`** | **Menedżer kolejności rysowania wg warstw** (rework FAZA 2): przenosi wszystkie obiekty wskazanej warstwy na wierzch albo spód i **zapisuje** — strzałki góra/dół, sort A→Z, zapisz/odtwórz układ. Działa na zapisanych rysunkach klienta. Native `DRAWORDER` robi per-obiekt — per-warstwa + persystencja układu to nasza wartość. | kod w *powertools* (`skrypty/GSAI_KOL.py`) | 🟡 **wskrzeszony reworkiem FAZA 2** (był na liście „wyleciało" sprzed przeróbki). Opis 2 wersje gotowy. W teście zespołu — re-werdykt Roberta przez build testerski (kiedyś zamknął wariant sprzed reworku). |
| **`GSAI_SKL`** | **Skala linii warstwy** — ustawia skalę rodzaju linii (gęstość kreskowania) dla całej warstwy naraz; natywnie tylko globalnie albo obiekt po obiekcie. | kod w *powertools* (`skrypty/GSAI_SKL.py`) | 🟡 zbudowane; w teście zespołu, na stronę po odbiorze praktyka. |
| **`GSAI_ODSWIEZ`** | **Serwis** — sprawdza, czy zestawienia liczone z rysunku (np. pola z `GSAI_POLA`) są aktualne; nieaktualne opisy zaznacza na czerwono, jednym poleceniem dla wszystkich narzędzi naraz. | kod w *powertools* (`skrypty/GSAI_ODSWIEZ.py`) | 🟡 narzędzie serwisowe; w teście zespołu. |
...
| **`GSAI_PRZEJ`** | Analiza przejezdności (swept-path): obwiednia pojazdu miarodajnego na trasie + ścięcie zakrętu (śmieciarka, naczepa, autobus). | kod w *powertools* | 🟡 **w testach zespołu, na stronę po odbiorze praktyka** (kandydat premierowy 15.08). |
...
| **`GSAI_TRASA`** 🆓 | Wskaż polilinię przyłącza → wykaz przeciętych działek (obrysy + numery) w rysunku. | bundle `GSAI-geoportal-test.zip` | 🟡 runtime-test zespołu ([#113](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/113)). **Obejście 162444/162495** (czyta wierzchołki przez ADS DXF grupa 10, gdy encja wraca jako klasa bazowa) — **POTWIERDZONE przez Tomasza na wczytanym pliku PL-2000** (2 przecięte działki). |
...
| Komenda | Co robi | Plik | Stan |
|---|---|---|---|
| **`GSAI_SCHRON`** 💰 | Checker budowli ochronnej (schron/ukrycie): wskaż zamkniętą polilinię strefy → pole (COM Area) → opcjonalnie liczba osób → sprawdza wymagania WT: min 1 m²/os; wyjścia (>50 os → ≥2, >1000 os → ≥2 poza strefę zagruzowania); wejścia (>300 os → ≥2); podział na strefy (ukrycie ≤300 os, schron S-1 ≤1000 os); dopuszczalność szybu (≤35 m² i ≤10 os); szer. drogi ewakuacyjnej 0,4 m/100 os. **Zakres v1: wymiarowy** (grubości przegród / wentylacja / dojście ≤500 m poza zakresem). Podstawa: rozp. MSWiA z 4.11.2025 (Dz.U. 2025/1548) + ust. z 5.12.2024 o ochronie ludności (Dz.U. 2024/1907). | bundle `GSAI-schron-test.zip` | 🟡 nowy moduł, przed odbiorem praktyka ([#114](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/114)). **Pole strefy przez obejście 162444/162495** (ADS DXF grupa 10, gdy encja wraca jako klasa bazowa) — **POTWIERDZONE przez Tomasza na wczytanym pliku PL-2000** (pole 52,43 m²). |
| **`GSAI_DACH`** 🎁 | Generator połaci dachu ze straight-skeleton: wskaż obrys → połacie, kalenice, krawędzie, opisy i strzałki spadków. Native-first + COM fallback (BUG-10-świadome). Generowania połaci z obrysu nie ma nikt. | kod w *powertools* (`GSAI_DACH.py` + `gsai_dach_core.py`, self-test 16/16) | ✅ **odebrany przez Roberta na produkcję** — [robert#16](https://github.com/init3-sentry/gstarcad-ai-robert/issues/16): „**moduł dachy super, m2 poprawione, texty czytelne** […] interface gra" (po wdrożeniu jego uwag o opisach/strzałkach/„m²"). Pełny pass zespołu na 5 typach dachu ([#100](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/100), Tomasz ✓). *Werdykt sprzed zaostrzenia protokołu na jawne „zamykamy" — Dawid potwierdził 27.08, że pochwała = zielone światło.* |
...
## ⏸ Odsunięte poza premierę 1.09 — wracają z fixem producenta

> Zbudowane, ale **świadomie zdjęte z premiery 1.09** z powodu buga po stronie GstarSoftu. **To nie jest „gotowe" ani „wycofane"** — wrócą do testów i katalogu, gdy producent dowiezie fix. Nie umieszczać na stronie.

| Komenda | Co robi | Powód odsunięcia | Kiedy wraca |
|---|---|---|---|
| **`GSAI_ORTOFOTO`** 🆓 | Podkład ortofotomapy jako raster (PZGIK/GUGiK). | Raster + bug producenta **BUG-05** + **downcast 162444** na słowniku obrazów (encja rastra wraca jako klasa bazowa). Decyzja Dawida 24.08: zdjęte z 1.09. | Z fixem GstarSoftu — I tydzień września. Wtedy retest zespołu i powrót do Suite GEO. |
| **`GSAI_DL`** 🆓 | Suma długości wskazanych obiektów + trwały opis na rysunku (`_OPIS`). Alias `GSAI_TLEN`. | 🔴 **BUG-10 / downcast 162444** — `length` pada na encjach wczytanych z **zapisanych** plików klienta (działa tylko na świeżym rysunku, bezużyteczny na realnych). Decyzja Dawida **2026-09-09: zdjęte z wydania** (usunięte z bundla, manifestu, bramki). | Z fixem GstarSoftu (162444) albo po przepisaniu odczytu na ADS/`entget` — wtedy retest zespołu i powrót. |
...
| **Podkład rastrowy z georeferencją** | Zrealizowane jako **`GSAI_GEORASTER`** (wiersz wyżej). |
