...
> - **`GSAI_TRASA` + `GSAI_SCHRON` — obejście buga 162444 (Python) / 162495 (COM)** potwierdzonego przez GstarSoft: czytają wierzchołki/pole przez **ADS DXF (grupa 10)**, gdy encja wraca jako klasa bazowa na wczytanym pliku. **Obejście NIEPOTWIERDZONE — w weryfikacji zespołu (Jakub/Tomasz) na plikach wczytanych.** Status bez zmian: w testach; NIE przenosimy do gotowych, dopóki obejście nie potwierdzone.
...
| **`GSAI_TRASA`** 🆓 | Wskaż polilinię przyłącza → wykaz przeciętych działek (obrysy + numery) w rysunku. | bundle `GSAI-geoportal-test.zip` | 🟡 runtime-test zespołu ([#113](https://github.com/init3-sentry/gstarcad-ai-zespol/issues/113)). **Obejście 162444/162495** (czyta wierzchołki przez ADS DXF grupa 10, gdy encja wraca jako klasa bazowa) — **NIEPOTWIERDZONE, w weryfikacji Jakub/Tomasz na plikach wczytanych.** |
